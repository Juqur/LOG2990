...
export class MouseService {
    ...
    async processClick(): Promise<number[]> {
        ...
        if (this.getCanClick()) {
            ...
            const differencesArray = await this.getDifferencesArray(position);
    ...
    async getDifferencesArray(position: number) {
        return await lastValueFrom(this.communicationService.postDifference('7', position));
    }
